    ...
    <div className="flex flex-col gap-2 w-full">
      ...
      >
        ...
        <input
          id={id}
          name={name}
          className={`flex-1 focus:outline-none w-full border-l-0 border-r-0 border-t-0 border-b-[#EAE7F1] bg-transparent text-lightDark placeholder:text-[#B6AACA] ${classNameOfInput}`}
          {...others}
          type={type== "password" ? showPassword ? 'text' : 'password' : type || "text"}
          onFocus={() => setFocused(true)}
          onBlur={() => setFocused(false)}
        />
        {!!endAdornment && <div className="text-lightDark" onMouseDown={() => setShowPassword(true)} onMouseUp={() => setShowPassword(false)}>{endAdornment}</div>}
      </div>
